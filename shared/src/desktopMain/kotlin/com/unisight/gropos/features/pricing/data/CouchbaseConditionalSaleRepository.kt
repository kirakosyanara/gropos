...
/**
 * CouchbaseLite implementation of ConditionalSaleRepository for Desktop (JVM).
 * 
...
class CouchbaseConditionalSaleRepository(
    ...
    override suspend fun getActiveRules(): List<ConditionalSale> = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseConditionalSaleRepository: Error getting active rules - ${e.message}")
            emptyList()
        }
    }
    ...
    override suspend fun getRuleById(ruleId: Int): ConditionalSale? = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseConditionalSaleRepository: Error getting rule by ID $ruleId - ${e.message}")
            null
        }
    }
    ...
    override suspend fun getRulesForProduct(branchProductId: Int, categoryId: Int?): List<ConditionalSale> = withContext(Dispatchers.IO) {
        try {
            // Get all active rules and filter in memory
            // (Couchbase Lite N1QL doesn't easily support array contains + OR logic)
            ...
            println("CouchbaseConditionalSaleRepository: Error getting rules for product - ${e.message}")
            emptyList()
        }
    }
    ...
    override suspend fun getAgeRestrictionRules(): List<ConditionalSale> = withContext(Dispatchers.IO) {
        try {
            // Get all active rules and filter for age restrictions
            val allRules = getActiveRules()
            ...
            println("CouchbaseConditionalSaleRepository: Error getting age restriction rules - ${e.message}")
            emptyList()
        }
    }
    ...
    override suspend fun getRequiredAgeForProduct(branchProductId: Int, categoryId: Int?): Int? = withContext(Dispatchers.IO) {
        try {
            ...
            // Return the highest age restriction among all applicable rules
            rulesForProduct
                .filter { it.isAgeRestricted }
                .maxOfOrNull { it.minimumAge ?: 0 }
                ?.takeIf { it > 0 }
        } catch (e: Exception) {
            println("CouchbaseConditionalSaleRepository: Error getting required age for product - ${e.message}")
            null
        }
    }
}
