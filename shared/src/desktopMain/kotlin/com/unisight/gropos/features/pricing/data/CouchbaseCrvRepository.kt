...
/**
 * CouchbaseLite implementation of CrvRepository for Desktop (JVM).
 * 
...
class CouchbaseCrvRepository(
    ...
    override suspend fun getAllCrvRates(): List<Crv> = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseCrvRepository: Error getting all CRV rates - ${e.message}")
            emptyList()
        }
    }
    ...
    override suspend fun getCrvById(crvId: Int): Crv? = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseCrvRepository: Error getting CRV by ID $crvId - ${e.message}")
            null
        }
    }
    ...
    override suspend fun getDefaultSmallContainerCrv(): Crv? = withContext(Dispatchers.IO) {
        try {
            // Per COUCHBASE_LOCAL_STORAGE.md: "CRV Under 24oz" for small containers
            val query = QueryBuilder.select(SelectResult.all())
                .from(DataSource.collection(collection))
            ...
            println("CouchbaseCrvRepository: Error getting default small CRV - ${e.message}")
            null
        }
    }
    ...
    override suspend fun getDefaultLargeContainerCrv(): Crv? = withContext(Dispatchers.IO) {
        try {
            // Per COUCHBASE_LOCAL_STORAGE.md: "CRV 24oz and Over" for large containers
            val query = QueryBuilder.select(SelectResult.all())
                .from(DataSource.collection(collection))
            ...
            println("CouchbaseCrvRepository: Error getting default large CRV - ${e.message}")
            null
        }
    }
}
