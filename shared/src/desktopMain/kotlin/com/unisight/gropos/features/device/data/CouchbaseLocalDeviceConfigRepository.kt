...
/**
 * CouchbaseLite implementation of LocalDeviceConfigRepository for Desktop (JVM).
 * 
...
class CouchbaseLocalDeviceConfigRepository(
    ...
    override suspend fun getHardwareConfig(environment: String): HardwareConfig? = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseLocalDeviceConfigRepository: Error getting hardware config - ${e.message}")
            null
        }
    }
    ...
    override suspend fun getBranchId(environment: String): Int? = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseLocalDeviceConfigRepository: Error getting branch ID - ${e.message}")
            null
        }
    }
    ...
    override suspend fun getBranchName(environment: String): String? = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseLocalDeviceConfigRepository: Error getting branch name - ${e.message}")
            null
        }
    }
    ...
    override suspend fun getApiKey(environment: String): String? = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseLocalDeviceConfigRepository: Error getting API key - ${e.message}")
            null
        }
    }
    ...
    override suspend fun getRefreshToken(environment: String): String? = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseLocalDeviceConfigRepository: Error getting refresh token - ${e.message}")
            null
        }
    }
    ...
    override suspend fun saveHardwareConfig(config: HardwareConfig, environment: String): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            collection.save(doc)
            println("CouchbaseLocalDeviceConfigRepository: Saved hardware config for $environment")
            Result.success(Unit)
        } catch (e: Exception) {
            println("CouchbaseLocalDeviceConfigRepository: Error saving hardware config - ${e.message}")
            Result.failure(e)
        }
    }
    ...
    override suspend fun saveRefreshToken(refreshToken: String, environment: String): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            collection.save(doc)
            println("CouchbaseLocalDeviceConfigRepository: Saved refresh token for $environment")
            Result.success(Unit)
        } catch (e: Exception) {
            println("CouchbaseLocalDeviceConfigRepository: Error saving refresh token - ${e.message}")
            Result.failure(e)
        }
    }
}
