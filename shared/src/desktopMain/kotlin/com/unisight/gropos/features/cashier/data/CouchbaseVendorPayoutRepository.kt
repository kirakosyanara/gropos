...
/**
 * CouchbaseLite implementation of VendorPayoutRepository for Desktop (JVM).
 * 
...
class CouchbaseVendorPayoutRepository(
    ...
    private val collection: Collection by lazy {
        ...
        // Create index on payoutDateTime for date range queries
        try {
            coll.createIndex(
                "payout_date_idx",
    ...
    override suspend fun savePayout(payout: VendorPayout): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            collection.save(doc)
            println("CouchbaseVendorPayoutRepository: Saved payout ${payout.id} for ${payout.vendorName}")
            Result.success(Unit)
        } catch (e: Exception) {
            println("CouchbaseVendorPayoutRepository: Error saving payout - ${e.message}")
            Result.failure(e)
        }
    }
    ...
    override suspend fun getPayoutById(payoutId: Long): VendorPayout? = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseVendorPayoutRepository: Error getting payout by ID - ${e.message}")
            null
        }
    }
    ...
    override suspend fun getPayoutsForDateRange(startDate: String, endDate: String): List<VendorPayout> = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseVendorPayoutRepository: Error getting payouts for date range - ${e.message}")
            emptyList()
        }
    }
    ...
    override suspend fun getPayoutsForVendor(vendorId: Int, limit: Int): List<VendorPayout> = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseVendorPayoutRepository: Error getting payouts for vendor - ${e.message}")
            emptyList()
        }
    }
    ...
    override suspend fun getPayoutsForStation(stationId: Int, limit: Int): List<VendorPayout> = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseVendorPayoutRepository: Error getting payouts for station - ${e.message}")
            emptyList()
        }
    }
    ...
    override suspend fun getTodayPayoutTotal(stationId: Int?): BigDecimal = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseVendorPayoutRepository: Error getting today's total - ${e.message}")
            BigDecimal.ZERO
        }
    }
    ...
    override suspend fun getUnsyncedPayouts(): List<VendorPayout> = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseVendorPayoutRepository: Error getting unsynced payouts - ${e.message}")
            emptyList()
        }
    }
    ...
    override suspend fun markAsSynced(payoutId: Long): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            if (doc != null) {
                ...
                println("CouchbaseVendorPayoutRepository: Marked payout $payoutId as synced")
            }
            Result.success(Unit)
        } catch (e: Exception) {
            println("CouchbaseVendorPayoutRepository: Error marking payout as synced - ${e.message}")
            Result.failure(e)
        }
    }
}
