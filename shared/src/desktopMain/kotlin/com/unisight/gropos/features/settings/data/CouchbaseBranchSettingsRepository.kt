...
/**
 * CouchbaseLite implementation of BranchSettingsRepository for Desktop (JVM).
 * 
...
class CouchbaseBranchSettingsRepository(
    ...
    override suspend fun getAllSettings(): BranchSettings = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            println("CouchbaseBranchSettingsRepository: Error getting all settings - ${e.message}")
    ...
    override suspend fun getSettingsForBranch(branchId: Int): List<BranchSetting> = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseBranchSettingsRepository: Error getting settings for branch - ${e.message}")
            emptyList()
        }
    }
    ...
    override suspend fun saveSetting(setting: BranchSetting): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            println("CouchbaseBranchSettingsRepository: Saved setting ${setting.type}")
            Result.success(Unit)
        } catch (e: Exception) {
            println("CouchbaseBranchSettingsRepository: Error saving setting - ${e.message}")
            Result.failure(e)
        }
    }
