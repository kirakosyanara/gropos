...
/**
 * CouchbaseLite implementation of TaxRepository for Android.
 * 
...
class CouchbaseTaxRepository(
    ...
    override suspend fun getAllTaxes(): List<Tax> = withContext(Dispatchers.IO) {
        try {
            ...
            android.util.Log.e("TaxRepository", "Error getting all taxes: ${e.message}")
            emptyList()
        }
    }
    ...
    override suspend fun getTaxById(taxId: Int): Tax? = withContext(Dispatchers.IO) {
        try {
            ...
            android.util.Log.e("TaxRepository", "Error getting tax by ID $taxId: ${e.message}")
            null
        }
    }
    ...
    override suspend fun getTaxByName(name: String): Tax? = withContext(Dispatchers.IO) {
        try {
            ...
            android.util.Log.e("TaxRepository", "Error getting tax by name '$name': ${e.message}")
            null
        }
    }
    ...
    override suspend fun getTaxesByIds(taxIds: List<Int>): List<Tax> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            android.util.Log.e("TaxRepository", "Error getting taxes by IDs: ${e.message}")
            emptyList()
        }
    }
}
