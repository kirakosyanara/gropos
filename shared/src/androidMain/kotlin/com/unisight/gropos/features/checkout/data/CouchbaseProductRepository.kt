...
class CouchbaseProductRepository(
    ...
    override suspend fun getByProductId(productId: Int): Product? = withContext(Dispatchers.IO) {
        try {
            ...
            Log.e(TAG, "Error getting product by productId $productId", e)
            null
        }
    }
    ...
    private fun parseNewFormatProduct(map: Map<String, Any?>): Product? {
        return try {
            ...
            Log.e(TAG, "Error parsing new format document", e)
            null
        }
    }
    ...
    override suspend fun insertProduct(product: Product): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            Log.d(TAG, "Upserted product ${product.branchProductId} - ${product.productName}")
            true
        } catch (e: Exception) {
            Log.e(TAG, "Error upserting product ${product.branchProductId}", e)
            false
        }
    }
