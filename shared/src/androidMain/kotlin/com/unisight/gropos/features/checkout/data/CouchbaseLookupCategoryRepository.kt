...
/**
 * CouchbaseLite implementation of LookupCategoryRepository for Android.
 * 
...
class CouchbaseLookupCategoryRepository(
    ...
    companion object {
        private const val TAG = "CouchbaseLookupCategoryRepo"
    }
    ...
    override suspend fun getCategoryById(id: Int): LookupCategoryWithItems? = withContext(Dispatchers.IO) {
        try {
            ...
            Log.e(TAG, "Error getting category $id", e)
            null
        }
    }
    ...
    override suspend fun saveCategory(category: LookupCategoryWithItems): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            Log.d(TAG, "Upserted category ${category.id} - ${category.name} (${category.items.size} items)")
            true
        } catch (e: Exception) {
            Log.e(TAG, "Error saving category ${category.id}", e)
            false
        }
    }
    ...
    override suspend fun clearAll() = withContext(Dispatchers.IO) {
        try {
            ...
            Log.d(TAG, "Cleared all categories")
        } catch (e: Exception) {
